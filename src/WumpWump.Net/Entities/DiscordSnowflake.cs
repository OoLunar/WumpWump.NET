...
namespace WumpWump.Net.Entities
{
    ...
    /// </summary>
    [JsonConverter(typeof(DiscordSnowflakeJsonConverter))]
    public readonly record struct DiscordSnowflake : IComparable<DiscordSnowflake>
    {
        /// <summary>
        ...
        public int CompareTo(DiscordSnowflake other) => other == default ? 1 : Value.CompareTo(other.Value);

        public static bool operator <(DiscordSnowflake left, DiscordSnowflake right) => left == default ? right != default : left.CompareTo(right) < 0;
        public static bool operator <=(DiscordSnowflake left, DiscordSnowflake right) => left == default || left.CompareTo(right) <= 0;
        public static bool operator >(DiscordSnowflake left, DiscordSnowflake right) => left != default && left.CompareTo(right) > 0;
        public static bool operator >=(DiscordSnowflake left, DiscordSnowflake right) => left == default ? right == default : left.CompareTo(right) >= 0;
